...
var.s1 = sddpVar(Gps,H) ;        % storage of pumped storage
var.p_pump1 = sddpVar(Gps,H) ;   % power pumped to pumped storage
var.p_prod1 = sddpVar(Gps,H) ;   % power produced by pumped storage

var.p1 = sddpVar(G, H) ;         % Production of conv generators
var.c1 = sddpVar(G, H) ;         % cost of conv generators at the coresponding production level
var.ls1 = sddpVar(nBus, H) ;     % load shedding at each bus
var.psh1 = sddpVar(nBus, H) ;    % production shedding at each bus

var.redispatchC1 = sddpVar(H);%

var.flow1 = sddpVar(nLines, H);  % power flow at each line
var.angle1 = sddpVar(nBus, H) ;  % angle at each bus

var.e = sddpVar(H);             %forecast error
var.ruc = sddpVar(G,H);
var.rdc = sddpVar(G,H);

lattice = compileLattice(lattice,@(scenario)nldsMOLookAhead(scenario,var,GeneratorConv,GeneratorPS,GeneratorWS,Loads,Bus,Lines,UC,H),params) ;
...
RuC = zeros(G,H,nForward);
RdC = zeros(G,H,nForward);
...
for  i = 1:nForward
    ...
    [objVec(i),~,~,solution] = forwardPass(lattice,myPath(i,1:H) ,params) ;
    %[objVec(i),~,~,solution] = forwardPass(lattice,ones(1,H) ,params) ;
    ...
    c1(:,:,i) = lattice.getPrimalSolution(var.c1 , solution);
    
    RuC(:,:,i) = lattice.getPrimalSolution(var.ruc , solution);
    RdC(:,:,i) = lattice.getPrimalSolution(var.rdc , solution);
    ...
    %loadsheddingCost(i) = loadshedding(i)*3000;
